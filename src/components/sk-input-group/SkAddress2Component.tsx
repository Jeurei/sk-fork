...
function isDefinedDestination(address: TCustomizableAddress | null) {
  return (
    address &&
    !address.isCustom &&
    (address.coords.lat !== 0 || address.coords.lon !== 0)
  );
}
...
@Component({})
export class SkAddress2OptionsComponent<T> extends tsx.Component<
  SkAddress2OptionsComponentProps<T>,
  SkAddress2OptionsComponentEvents<T>,
  { option: SkAddress2OptionsComponentSlotProps<T> }
> {
  @Prop()
  ...
  public render() {
    let options = this.options.map((x, i) => {
      let classes = {
        ...
        "sk-address2-options__option_selected": this.comparator(
          this.selected,
          x
        ),
      };
      ...
      return this.$scopedSlots.option ? (
        ...
        <div
          key={key}
          class={classes}
          onMouseenter={() => this.setCursor(i)}
          onMousedown={tsx.modifiers.prevent(() => this.$emit("select", x))}
        >
          {x}
        </div>
      );
...
export class SkAddress2Component extends tsx.Component<{}> {
  ...
  @Watch("value")
  protected __onValueSpecified(
    value: TCustomizableAddress | null,
    oldValue: TCustomizableAddress | null
  ) {
  ...
  protected comparator(
    a: TCustomizableAddress | null,
    b: TCustomizableAddress | null
  ) {
    if (a && a.isCustom && b && b.isCustom) {
      return (
        a.point.replace(/(^\s+|\s+$)/g, "") ===
        b.point.replace(/(^\s+|\s+$)/g, "")
      );
    }
  ...
  protected async updateOptions(query: string) {
    ...
    try {
      ...
      let options = await this.completeService.findInDistrict(
        this.city,
        this.districtCities,
        trimmed,
        this.includeCityOptions
      );
      if (query !== this.query) {
        return;
      }
    ...
    return (
      <div class={this.classes}>
        <input
          ...
          value={this.query}
          onInput={(e: Event) =>
            this.setQuery((e.target as HTMLInputElement).value, true, true)
          }
        ...
        <SkAddress2OptionsComponent
          ...
          scopedSlots={{
            option: (
              option: SkAddress2OptionsComponentSlotProps<TCustomizableAddress>
            ) => (
              <div
                key={option.key}
                class={{
                  ...
                  "sk-address2__option_final": isDefinedDestination(
                    option.value
                  ),
                  "sk-address2__option_notfinal": !isDefinedDestination(
                    option.value
                  ),
                }}
              ...
              >
                {option.value.citymeta &&
                option.value.address_source === "custom"
                  ? [
                      <div class="sk-address2__option-street">
                        {option.value.citymeta.name}
                      </div>,
                      <div class="sk-address2__option-city"></div>,
                    ]
                  : [
                      <div class="sk-address2__option-street">
                        {getQueryByAddress(option.value)}
                      </div>,
                      <div class="sk-address2__option-city">
                        {getDescriptionByAddress(option.value)}
                      </div>,
                    ]}
              </div>
            ),

...
import { TIncompleteDestination, TDestination, alterDestinationIds, CarType, Wish, Overprice, alterDestinationId, TAddress, TRoute, getAddressId, TCustomizableAddress } from '@model';
import { Api } from '@api';
...
export class SkFormModule extends VuexModule {
    ...
    public sourceTime: string = (() => {
        ...
        return `${date.getFullYear()}-${date.getMonth() + 1}-${date.getDate()} - ${date.getHours()}:${date.getMinutes()}`.replace(/(^|:|[^0-9])([0-9])(?=$|[^0-9])/g, '$10$2');
    })();
    ...
    public setDestinations(destinations: TIncompleteDestination[]) {
        ...
        if (this.destinations[0].address && this.destinations[0].address.citymeta && this.destinations[0].address.citymeta !== skConfigModule.city) {
    ...
    public async createOrder() {
        ...
        if (this.useSourceTime) {
            let matches = /^(\d{4})-(\d{2})-(\d{2}) - (\d{2}):(\d{2})$/.exec(this.sourceTime);
            if (!matches) {
        ...
        let offset = Math.floor(skConfigModule.serverTimezone - skConfigModule.city.timezone);
        ...
        try {
            ...
            orderId = await Api.trash.createOrder2({
                ...
                server_time_offset: skConfigModule.city.timezone - skConfigModule.serverTimezone,
                source_time: dateStr,
    ...
    public async updateTariff() {
        let promise = (async () => {
            ...
            if (route) {
                ...
                let addresses = this.destinations.map(x => x.address).filter(x => !!x) as TAddress[];
    ...
    public async updatePrice() {
        let promise = (async () => {
            ...
            if (route) {
                ...
                return await Api.trash.calcOrderCost2WithoutRoute(tariffId, addresses, orderParams);
            }
        })();
...
store.watch(
    () => skFormModule.destinations.map(x => x.address).map(x => x ? getAddressId(x) : 'null').join('\n'),
    () => {
...
store.watch(
    () => {
        ...
        if (route) {
            ...
            let addresses = skFormModule.destinations.map(x => x.address).filter(x => !!x) as TAddress[];
            if (addresses.length < 2) {
                coords = [];
            }
            coords = addresses.map(x => x.coords);
...
store.watch(
    () => {
        ...
        if (route) {
            ...
            let addresses = skFormModule.destinations.map(x => x.address).filter(x => !!x) as TCustomizableAddress[];
            if (addresses.length < 2) {
                routeKey = '';
            } else {
...
store.watch(
    () => skModule.skOrderId,
    () => { skFormModule.clear(); skConfigModule.reset(); },
    {
        immediate: false,
        deep: false,
    }

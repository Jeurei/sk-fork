...
export class SkModule extends VuexModule {
  ...
  public get overprice() {
    ...
    let overprices = this.skOrderState.order_params
      .map((x) =>
        skConfigModule.overpriceOptionsAfterOrderCreated.find((y) => y.id === x)
      )
      .filter((x) => !!x);
  ...
  public async applyOverprice(overprice: Overprice | null) {
    ...
    let currentOverprices = currentOrderParams
      .map((x) =>
        skConfigModule.overpriceOptionsAfterOrderCreated.find((y) => y.id === x)
      )
      .filter((x) => !!x) as Overprice[];
    let order_params = currentOrderParams.filter(
      (x) => currentOverprices.findIndex((y) => y.id === x) === -1
    );

    let maxOverprice = currentOverprices.reduce(
      (acc, x) => (!acc ? x : x.value > acc.value ? x : acc),
      null as Overprice | null
    );
    ...
    if (
      maxOverprice !== null &&
      (overprice === null || maxOverprice.value > overprice.value)
    ) {
  ...
  public async skLoadOrderState() {
    ...
    try {
      ...
      if (
        orderState.state_kind !== "finished" &&
        orderState.state_kind !== "aborted"
      ) {
        try {
          let currentOrders = await Api.trash.getCurrentOrdersByClientId(
            orderState.client_id
          );
  ...
  public async skLoadPrice() {
    ...
    try {
      let info = await Api.trash.getInfoByOrderId(orderId, [
        "DISCOUNTEDSUMM",
        "DRIVER_TIMECOUNT",
      ]);
  ...
  public async skApproveOrder() {
    ...
    let elem = document.getElementsByClassName(
      "sk-order-status-controls__connect-client-and-driver"
    )[0];
...
async function refreshStateAndSchedule() {
  ...
  try {
    if (skConfigModule.ready) {
      await Promise.all([skModule.skLoadOrderState(), skModule.skLoadPrice()]);
    }
  } catch (e) {
    console.error(e);
  }
...
store.watch(
  () =>
    skModule.skOrderState
      ? getRouteKey(getRouteCoords(skModule.skOrderState))
      : "",
  () => {

...
import { LMap, LTileLayer, LMarker, LPopup, LPolyline, LIcon } from 'vue2-leaflet';
...
import { SkDialogServiceComponent, SkDialogComponent } from '../sk-dialog-service/SkDialogService';
...
export class AppComponent extends tsx.Component<AppComponentProps> {
    ...
    protected getDialogService() { return this.$refs['dialogService']; }
    ...
    protected async __onCitySpecified(newCity: string) {
        try {
            ...
            items = items.filter(x => x.kind === 'point').filter(x => x.coords.lat !== 0 && x.coords.lon !== 0);
            if (items.length > 0) {
    ...
    protected get bounds(): [number, number][] {
        ...
        return skConfigModule.city.boundaries.length > 0 ? skConfigModule.city.boundaries : (this.cityCoords ? [this.cityCoords] : []);
    }
    ...
    protected getFormBounds(): [number, number][] {
        ...
        return [
            // all stops
            ...skFormModule.destinations.filter(x => x.address).map(x => [x.address!.coords.lat, x.address!.coords.lon]),

            // route
            ...(skFormModule.route ? skFormModule.route.full_route_coords.map(x => [x.lat, x.lon]) : []),
    ...
    protected getOrderBounds() {
        ...
        if (skModule.skRoute) {
            routeCoords = skModule.skRoute.full_route_coords.map(x => [x.lat, x.lon]);
        }

        return [
            // all stops
            [skModule.skOrderState.source_lat, skModule.skOrderState.source_lon],
            [skModule.skOrderState.destination_lat, skModule.skOrderState.destination_lon],
            ...
            // crew
            ...(skModule.skOrderState.crew_coords ? [[skModule.skOrderState.crew_coords.lat, skModule.skOrderState.crew_coords.lon]] : []),
        ].filter(x => !(x[0] === 0 && x[1] === 0)) as [number, number][];
    }
    ...
    protected renderFormObjects() {
        ...
        return [
            ...
            skFormModule.route
            ? <LPolyline
                lat-lngs={skFormModule.route.full_route_coords.map(x => [x.lat, x.lon])}
                color="#f00"
            />
            : null,

            // all stops
            ...skFormModule.destinations.filter(x => x.address && !(x.address.coords.lat === 0 && x.address.coords.lon === 0)).map((x, i) => (
                <LMarker
                    key={x.id}
    ...
    protected renderOrderObjects() {
        ...
        return [
            ...
            skModule.skRoute
            ? <LPolyline
                lat-lngs={skModule.skRoute.full_route_coords.map(x => [x.lat, x.lon])}
                color="#f00"
            />
            : null,
            ...
            ? <LMarker
                latLng={latLng(skModule.skOrderState.crew_coords.lat, skModule.skOrderState.crew_coords.lon)}
                icon={icon({
                    iconUrl: 'data:image/svg+xml,%3Csvg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 16 16"%3E%3Ccircle cx="8" cy="8" r="7" stroke="%23ff350e" stroke-width="2" fill="%23fff" /%3E%3C/svg%3E',
                    iconSize: [16, 16],
                    iconAnchor: [8, 8]
                })}
            />
        ...
        return (
            ...
            >
                <div class="sk-app__panel">
                    <SkHeaderComponent orderState={this.orderState} orderStatus={this.orderState} />
                    {(
                        !this.orderState
                            ? <SkOrderFormComponent style="margin: 10px 0;" />
                            : <SkOrderStatusComponent orderState={this.orderState} car={skModule.skCar} canChangeOverprice={skModule.canChangeOverprice} style="margin: 10px 0;" />
                    )}
                ...
                <div class="sk-app__map">
                    <LMap
                        ...
                        bounds={latLngBounds(this.bounds)}
                    >
                        <LTileLayer
